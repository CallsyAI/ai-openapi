...
interface ApiReadmeOptions {
    branch?: string
    filename?: string
}

export default class ReadmeIntegration {
    private readonly BASE_URL = 'https://api.readme.com/v2'
    private readonly axiosInstance: AxiosInstance
    private readonly branch: string
    private readonly filename: string

    public constructor(apiKey: string, options?: ApiReadmeOptions) {
     ...
     */
    public async createApiDefinition(spec: JsonDict): Promise<JsonDict> {
        const formData = this.buildFormData(spec)
        const endpoint = `/branches/${this.branch}/apis`
        const config = {headers: formData.getHeaders()}
        const response = await this.axiosInstance.post<JsonDict>(endpoint, formData, config)
     ...
     */
    public async updateApiDefinition(spec: JsonDict): Promise<JsonDict> {
        const formData = this.buildFormData(spec)
        const endpoint = `/branches/${this.branch}/apis/${this.filename}`
        const config = {headers: formData.getHeaders()}
        const response = await this.axiosInstance.put<JsonDict>(endpoint, formData, config)
        return response.data
    }
     ...
     */
    public async getApiDefinition(): Promise<JsonDict> {
        const endpoint = `/branches/${this.branch}/apis/${this.filename}`
        const response = await this.axiosInstance.get<JsonDict>(endpoint)
        return response.data
    }
     ...
     */
    public async createOrUpdateApiDefinition(spec: JsonDict): Promise<JsonDict> {
        try {
